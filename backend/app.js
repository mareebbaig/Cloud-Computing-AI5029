...
const { docClient, TABLE_NAME, initializeDynamoDB } = require('./dynamodb-config');
...
async function startServer() {
    ...
    const dynamoInitialized = await initializeDynamoDB();
    
    if (!dynamoInitialized) {
        console.error('❌ Failed to initialize DynamoDB. Please ensure DynamoDB Local is running on port 8000.');
        process.exit(1);
    }
    ...
    app.listen(PORT, () => {
        ...
        console.log(`   GET    http://localhost:${PORT}/notes/:id - Get note by ID`);
        console.log(`   POST   http://localhost:${PORT}/notes     - Create new note`);
    });
}
